...
|---|---|---|
| Hostname (`steamlens.ardabasarici.dev`, `hr.`, `hr-w1.`, `hr-w2.`, `leave-agent.`) | platform: DNS record + site stanza | application README, links |
...
| Cloudflare Access in front of the world sites (`hr-w1`, `hr-w2`) | its own design step | it adds a service-to-service trust boundary (the leave agent's machine path needs a service token), not a browser login toggle |
