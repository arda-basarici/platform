...
moved on zero-diff plans, and the edge hardening that followed was applied from code.
The VPS is rebuildable from a blank host by a five-role Ansible play, proven on a
fresh one and drilled end to end.

The honest caveats: a single operator applies from a laptop; the live VPS was built by
...
databases has a backup, the other two holding regenerable placeholder data until the
agent's first generated world is data worth keeping.

> [!IMPORTANT]
> **First rebuild-and-restore drill (2026-08-30): blank host and blank control node →
> SteamLens serving its restored production backup in ~1 hour.** From the public
> repositories, the password vault, and the Google account: nothing from the old
> machines survived into the rebuild. Procedure and stamps:
> [`runbooks/box-rebuild.md`](runbooks/box-rebuild.md).
...
flowchart TD
    ...
    BOX --> HR["hr. / hr-w1. / hr-w3. → Frappe HR"]
...
| The box from a blank host: five roles in dependency order + the acceptance script | `ansible/` | passes on a blank host (2026-08-28, Debian 12; 2026-08-30 rebuild drill, Debian 13); compared against the live box in check mode (2026-08-30, ten differences, all classified), not yet replayed |
| Per-tenant adapters: site stanzas, backup units, the contract each tenant and the platform agree on | `projects/<name>/` | two tenants: `steamlens/` (the SteamLens site and its nightly backup) and `leave-impact/` (the Frappe HR sites on the box; the agent's own AWS host is the stack above) |
| The checks CI and the laptop share: the three pinned copies of Cloudflare's IPv4 ranges against each other and the published list | `scripts/` | weekly in CI against the published list |
...
| Runbooks, written when first exercised | `runbooks/` | add-a-tenant · the Ansible test host · replace the app host · box rebuild + restore |

## Proven, not asserted

| Claim | How it was tested | The honest caveat |
|---|---|---|
| The whole recovery path works from nothing | the rebuild drill (2026-08-30, [`runbooks/box-rebuild.md`](runbooks/box-rebuild.md)): a fresh control node (the age identity restored from the password vault and verified against the recorded recipient) and a blank Debian 13 host, taken to SteamLens serving restored data; the newest backup pulled through a *fresh* `drive.file` OAuth authorization, no credential from the old box or old workstation; all four table counts matching the object's recorded evidence digit for digit. About an hour end to end, first unrehearsed run, exact stamps in the runbook | the drill host was EC2, not netcup; DNS cutover and CD re-arming are listed in the runbook, not exercised; the fresh-OAuth path lives only while rclone's shared client id does (retirement announced for 2026) |
| The app host's failure reaches a person | two CloudWatch status-check alarms (system: recover + notify; instance: notify) on one SNS topic with a confirmed e-mail subscriber, applied 2026-08-30 as `2 imported, 5 added, 2 changed, 0 destroyed` (the two imports: the account's default cost-anomaly monitor and its subscription, delivery moved from a daily digest to the topic); the path proven by forcing the instance alarm into ALARM. The first try was refused (`CloudWatch Alarms is not authorized to perform: SNS:Publish` in the alarm history; the topic policy had named the account, not the CloudWatch service principal), one statement fixed it, and the second try delivered the ALARM and OK mails | the subscription's confirmation is a click in a mailbox Terraform cannot see; the box's host has no equivalent, only the external monitors |
| The SteamLens backup restores, and keeps restoring | the newest nightly object pulled back from Drive on the box (2026-08-30): `integrity_check` ok, 30.7 MB, and four table counts compared against the live store opened read-only, every one above the backup's by one day of writes (the counts: [`projects/steamlens/README.md`](projects/steamlens/README.md)); Drive read back holding exactly the 7 dailies + 4 Sunday weeklies the script prunes to. The same check now runs monthly from a timer (`restore-check.sh`, first of the month, an hour after the backup) and pings its own dead-man only on a pass; its first hand run the same evening printed the same counts | the swap-in was exercised at the 2026-08-30 rebuild drill on a throwaway host (the first row); on the live box it remains a runbook step; nightly cadence means a real restore loses up to a day |
...
| The box is rebuildable from the repository | a blank Debian 12 cloud host reached a passing `verify.sh` (sshd key-only, ufw 22 only, the Cloudflare-only chain, the proxy answering every hostname) from the play alone; repeated at the rebuild drill (2026-08-30) on a blank Debian 13 host, the box's own major: 88 s build, `changed=0` second run, `verify.sh` 26/26 | the live box has not been replayed |
...
| The hand-made edge is fully described by code | every import (8 records, 3 settings, 2 rulesets, bot control) landed as `N to import, 0/0/0`, then `No changes`; what followed was created from code (the three no-mail records, the TLS 1.2 floor, HSTS, DNSSEC) and verified live (a TLS 1.0 handshake refused; the zone signed at Cloudflare and, since 2026-09-14, its DS record at the `.dev` registry with answers validating at two public resolvers) | only settings that were set on purpose entered code; Cloudflare's defaults stay unmanaged by design |
...
| Both stacks plan clean | dated plans: leave-impact-prod `No changes` (2026-09-02); edge `No changes` (2026-09-15, the day after the registry's DS record landed; from 2026-08-28 until then the edge carried exactly one in-place change, the DNSSEC `status pending → active`, that waited on that record). The first plan of 2026-09-02 had found a queued host replacement: a docs commit had reworded two comments inside the boot template, and the rendered template is the instance's identity (`instance.tf` says so now); restored to the booted bytes, `No changes` | laptop-side, on demand |
...
- `terraform fmt` and a per-stack `validate` with the lock file read-only;
- ShellCheck on every shell script a host runs and on the cloud-init script
  rendered by the real `templatefile`;
- `ansible-playbook --syntax-check` with the play's collections pinned;
- `caddy validate` on the box layout with every tenant file imported and a
  throwaway pair where the Origin CA pair sits (the pass is the adapted config
  naming every declared hostname, never the word "valid" alone);
- the three pinned copies of Cloudflare's IPv4 ranges diffed against each other,
  and weekly against the published list.

Actions are pinned by commit and moved by Dependabot pull requests, never by aging.
`plan` and `apply` never leave the laptop: a CI plan would need a read identity over
state and every provider, and its output does not belong in a public pull request.
The real inventory and the local `.tfvars` are gitignored; the committed `.example`
files show the shape.
...
the author's own terminal. Drafted changes reached production only through the same
acceptance as everything else here: a plan read before an apply, a role proven on a
blank host before it targets the box, a validator's output read rather than its
verdict trusted.
